...
import { BaiduMapTool } from './baidu-map-tool.js';
import { GpsTool } from './gps-tool.js';
...
export class ToolManager {
    ...
    registerDefaultTools() {
        ...
        this.registerTool('baiduMap', new BaiduMapTool());
        this.registerTool('gps', new GpsTool());
    }
    ...
    getToolDeclarations() {
        ...
        this.tools.forEach((tool, name) => {
            if (tool.getDeclaration && typeof tool.getDeclaration === 'function') {
                const declarations = tool.getDeclaration();
                if (Array.isArray(declarations)) {
                    allDeclarations.push(...declarations);
                } else {
                    Logger.warn(`Tool ${name} getDeclaration() did not return an array`);
                }
            }
        });
    ...
    async handleToolCall(functionCall) {
        ...
        } else if (name === 'get_location_info' || name === 'get_driving_directions') {
            tool = this.tools.get('baiduMap');
        } else if (name === 'get_current_gps_location') {
            tool = this.tools.get('gps');
        } else {
